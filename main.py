...
parser.add_argument('--hw', type=str, default='cpu',
                    help = 'Hardware to use: cpu, cuda', required = False)
parser.add_argument('--cnnParams', default='', required=False,
                    help="provide cnn params to avoid the training before the encoding" )
...
filepaths = glob(ply_dir+'*.ply')[0:(args.Nfr)]
...
if args.cnnParams == '':
    ...
    optimizer = torch.optim.Adam(params=mdl.parameters(), lr=init_lr) # busqueda de minimo = optimizacion
...
if args.cnnParams != '':
    mdl = model_type(device)
    mdl.load_state_dict(torch.load(args.cnnParams))
    mdl.eval()
    ctx_type=mdl.ctx_type
    CL_mdl,nws=compute_model_CL(mdl.state_dict(), 32)
...
if slice_pc: # single pc only
    ...
    for i_slice,sGT in enumerate(sGTs):
        if len(sGT)>0:
            ...
            os.mkdir(bs_dir)  
            _,te = ENCODE_DECODE(1,bs_dir,mdl,ori_level-1,sGT,device=torch.device(hw))
            time_spente+=te
            CL+= get_dir_size(bs_dir)

            if decode:
                dec_sGT,td = ENCODE_DECODE(0,bs_dir,mdl,ori_level-1,device=torch.device(hw))
                time_spentd+=td
    ...
    for ifile,filepath in enumerate(filepaths[ifile_start:]):
        ...
        _,time_spente = ENCODE_DECODE(1,bs_dir,mdl,ori_level,GT, device = torch.device(hw))
        ...
        if decode:
            dec_GT,time_spentd = ENCODE_DECODE(0,bs_dir,mdl,ori_level, device=torch.device(hw))
            ...
            if args.cnnParams != '':
                print('mean_times:'+str(np.around(mean_tse/nframes,4))+'  '+str(np.around(mean_tsd,4)))
            else:
...
if args.cnnParams == '':
    ...
    lines.append('enc time: ' + str(np.around(meantimes[0],1))+'s')
